...
void read_rom_termometr(uint8_t *mas){
        ...
        for( uint8_t i =0; i<8; i++){
                mas[i]= read_byte_termometr(); //записывает массив из 1(0х01))
        }
        sei();
}
...
void read_scrpad(uint8_t *data){
        ...
        for( uint8_t i =0; i<9; i++){
                data[i]= read_byte_termometr(); //ничего вообще тут нет
                // uart_put("mas[i]", 6);
                // uart_put(data[i], sizeof(data[i]));
        }
        sei();
}
